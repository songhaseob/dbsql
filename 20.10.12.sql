...
3. 속성 정의
    ...
    식별자 속성 : entity 구분 지어줄 수 있는 속성(학생 - 학번, 사원 - 사번)
    일반 속성 : 업무분석을 통해 정의 된 속성
        ...
        파생 속성 : 다른 속성으로 부터 계산된 값
                   ...
                   엔터티 : 게시글,첨부파일
                           ...
                           ==> 쿼리를 매번 이렇게 귀찮아서 게시글 엔티티에 첨부파일 개수라는
                           컬럼을 만들고, 첨부파일 수가 수정이 될 때마다 첨부파일,게시글
                           엔터티 동시에 수정
                           ==>반정규화, 로직을 잘못 작성하게 되면 첨부파일 테이블의 행 개수
                           와, 게시글 엔터티의 첨부파일 수가 불일치 할 수도 있다
...
4. 관계 정의
   ...
   관계구성
    ...
    2. 차수 : 몇개의 엔터티와 연관 (대응) 되는지
             ...
             만약 설계시에 N:N 관계과 나오게 되면 추가적인 관계 엔터티를 통해
             N:N 관계를 1:N 관계로 해소를 해야한다
             
            ex) 사원은 하나의 부서에 속한다
                상품은 여러개의 주문을 통해 주문될 수 있다
                학생은 여러개의 교과목을 수강할 수 있다
    3. 옵션여부 : 관계에 연결된 상태 엔터티에 반드시 존재해야되는지 여부
...
Super Type / sub Type
java의 클래스 상속처럼 공통 분모를 Surper Trype entity에 설계하고
sub type마다 필요로 하는 속성은 별도로 관리

ex) 사원의 종류가 내근사원,설계사, 대리점 3개로 구분되고 
    3분류가 공통으로 갖고있는 속성과,별도로 같는 속성이 있을 때
    공통 속성은 SuperType에 
    개별속성은 subType을 통해 관리 할 수 있다
...
교과목으로는 과목명, 수강 대상학년, 교재정보들을 관리 하려고 한다
...
2 정규형 : 주 식별자에 종속이지 않은 속성을 분리
          ...
          (학생, 수강, 강좌 3개의 엔터티가 있고, 수강 엔터티는 학생번호, 강좌번호
           두개의 복합식별자로 이루어져 있었고, 일반속성으로 학생이름이 있었음
           학생이름은 학생번호 식별자 속성에만 종속이 되고, 강좌변호와는 관련이 없음
...
집전화번호 02~063 - XTZ[T]-QWER
핸드폰번호 0110-XYZT-QWER
