...
WHERE sal > (SELECT AVG(sal)
             FROM emp
             WHERE deptno = e.deptno);  --상호연관 메인쿼리에있는 컬럼을 써도된다


DESC dept;
INSERT INTO dept VALUES (99, 'ddit', 'daejeon');  ---데이터 추가 

sub4]
SELECT *
FROM dept
WHERE deptno NOT IN(SELECT deptno
                    FROM emp); -- emp테이블에 없는 99 40이 출력
...
교집합 : A ^(U거꾸로) B = {1, 5} (교환법칙 성립 A ^ B == B ^ A)
...
위아래 집합이에서 736번 사번은 중복되므로 한번만 나오게 된다. 전체행은 3건
SELECT empno, ename
FROM emp
...
UNION ALL 연산자는 중복제거 단계가 없다. 총데이터 4개의 행
SELECT empno, ename
FROM emp
WHERE empno IN (7369,7566)
UNION ALL             
...
두집합의 공통된 부분은 7369행 밖에 없음 : 총데이터 1행
SELECT empno, ename
FROM emp
WHERE empno IN (7369,7566)
INTERSECT             
