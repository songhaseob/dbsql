...
하나의행이 조회되어야 하는 상황에서 여러개의 행이 반환된 경우  (예외처리가 없을 때)
SET SERVEROUTP ON;
...
오라클은 10번 인자로 getDeptName실행 했을 때 ACCOUNTING 이라는 결과값을 기억(캐싱)
...
create or replace PACKAGE names IS 
...
Trigger(방아쇠) : 설정한 이벤트에 따라 실행할 로직을 담은 객체
    웹 : 로그인 버튼 클릭
        ==> 사용자 아이디 id input 값과, 비밀번호 input 값을
            파라미터로 서버로 전송
    db : 테이블의 데이터가 추가되거나 변경되었을 때 pl/sql블럭을
         실행
         uisers 테이블에 사용자 비밀번호가 변경 되었을 때
...
트리거
    ...
    장점(시스템을 신규로 만드는 사람) 
        : users테이블에 비밀번호를 바꾸는 로직은 작성을 해야한다
          users_histtory에 이력을 남기는 로직(java코드)은 작성하지 않아도 됨
    단점(시스템을 유지보수하는 사람) 
