...
1. 인위적으로 여러개의 행을 만들기
   ...
   EX : '202010' ==> 31
        ...
        해당 날짜의 요일에 따라서 새롭게 만든 7개 컬럼(일~월)중 날짜 위치 시킨다
...
참조타입 : 변수 타입을 테이블의 컬럼 정보를 통해 선언
          ...
          ==> 특정 테이블 컬럼의 타입을 참조하여 선언
              해당 컬럼의 타입이 변경이 되더라도 PL/SQL 코드는 수정 하지 않아도 됨
DECLARE 
...
BEGIN
    SELECT ename, empno, dname INTO v_ename, v_empno, v_dname
    FROM emp,dept
    WHERE empno = p_empno AND dept.deptno = emp.deptno;
