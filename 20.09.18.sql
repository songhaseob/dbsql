...
테이블을 생성하는 문법
system - 
CREATE TABLE [오라클사용자].테이블명(컬럼명 컬럼의 데이터 타입,
                                   컬럼명2 컬럼의 데이터 타입2..........);
...
신규테이블에 데이터 입력
ranger_no - 1, ranger_nm = 'brown', reg_dt = 현재날씨, 시간)
...
2. 문자 : VARCHAR2(사이즈-byte), CHAR(X)
          ...
          VARCHAR2(2000) : 최대2000BYTE를 담을 수 있는 문자 타입
          java - 2byte, oracle xe 11g - 3byte
          
          CHAR(1~2000byte), 고정길이 문자열
          CHAR(5) 'test'
          tests는 4byte고 char(5) 5byte이기 때문에
          남은데이터 공간에 공백문자를 삽입함
          'test '
          'test' == 'test '
3. 날짜 : DATE - 7byte 고정
...
제약조건은 4가지가 존재, 그중에 한가지 일부를 별도의 키워드로 제공
ORACLE에서 만들수 있는 제약조건이 5가지

1. NOT NULL : 컬럼에 반드시 값이 들어가게 하는 제약조건
2. UNIQUE : 해당컬럼에 중복된 값이 들어오는 것을 방지하는 제약조건
...
UNIQUE constraint (hssong.SYS_C007093) violated
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
...
PRIMARY KEY (deptno, dname) 설정에 따라 데이터가 입력될수 있다
...
INSERT INTO dept_test VALUES (90, 'ddit2', 'daejeon'); --두개컬림이 같을때 동일한 데이터는 안됨
...
UNIQUE 제약조건: 값의 중복이 없도록 방지하는 제약조건, 단 NULL을 허용 한다
U_테이블명_컬럼명
...
2. 참조 무결성 생성시 OPTION - ON DELETE SET NULL
              삭제시 참조 하고 있는 자식테이블의 컬럼을 NULL 만든다
3. 참조 무결성 생성시 OPTION - ON DELETE SET CASCADE
              삭제시 참조 하고 있는 자식테이블의 데이터도 같이 삭제    
...
CREATE TABLE emp_test (
    ...
    gender VARCHAR2(1) CONSTRAINT CHECK ( sal > 0),
);
...
***** 데이터 타입을 바꾸는 것은 데이터가 없을 때는 상관 없지만
      데이터가 있을 경우는 사이즈를 늘리는것을 제외하고는 불가능
...
2. emp_test 테이블의 empno컬럼 PRIMARY KEY 제약 조건 추가
